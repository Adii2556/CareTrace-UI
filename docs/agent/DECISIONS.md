...
| 2026-09-22 | Progressively enhance only authenticated GET navigation with pinned HTMX, while disabling its local history cache | Keep Django URLs/forms as the source of truth, avoid duplicating the shell on each navigation and prevent medical page snapshots from being stored in `localStorage` | Accepted |
